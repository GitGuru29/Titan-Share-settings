...
void AudioBackend::applySpatialAudio() {
    ...
    // Width 0–100 → cross-mix gain 0.0–0.45 (keeps mono compatibility)
    double crossGain  = (m_spatialWidth / 100.0) * 0.45;
    double directGain = 1.0 - crossGain * 0.4;
    // Haas delay: 0–20 ms proportional to width (in samples @ 48 kHz)
    int delaySamples = qMax(1, static_cast<int>(m_spatialWidth * 960 / 100));

    QString config;
    QTextStream ts(&config);
    ts << "# ArchTitan Spatial Audio — generated by ArchTitan Settings\n";
    ts << "# Width: " << m_spatialWidth << "\n\n";
    ts << "context.modules = [\n";
    ts << "    { name = libpipewire-module-filter-chain\n";
    ts << "        flags = [ nofail ]\n";
    ts << "        args = {\n";
    ts << "            node.description = \"ArchTitan Spatial\"\n";
    ts << "            media.name       = \"ArchTitan Spatial\"\n";
    ts << "            filter.graph = {\n";
    ts << "                nodes = [\n";
    ts << "                    { type = builtin label = copy  name = copyL }\n";
    ts << "                    { type = builtin label = copy  name = copyR }\n";
    ts << QString("                    { type = builtin label = delay name = delayL\n"
                  "                        control = { \"Max Delay\" = 1.0 \"Delay\" = %1 } }\n").arg(delaySamples);
    ts << QString("                    { type = builtin label = delay name = delayR\n"
                  "                        control = { \"Max Delay\" = 1.0 \"Delay\" = %1 } }\n").arg(delaySamples);
    ts << QString("                    { type = builtin label = mixer name = mixL\n"
                  "                        control = { \"Gain 1\" = %1 \"Gain 2\" = %2 } }\n")
          .arg(directGain, 0, 'f', 4).arg(crossGain, 0, 'f', 4);
    ts << QString("                    { type = builtin label = mixer name = mixR\n"
                  "                        control = { \"Gain 1\" = %1 \"Gain 2\" = %2 } }\n")
          .arg(directGain, 0, 'f', 4).arg(crossGain, 0, 'f', 4);
    ts << "                ]\n";
    ts << "                links = [\n";
    ts << "                    { output = \"copyL:Out\"  input = \"mixL:In 1\" }\n";
    ts << "                    { output = \"copyR:Out\"  input = \"delayR:In\" }\n";
    ts << "                    { output = \"delayR:Out\" input = \"mixL:In 2\" }\n";
    ts << "                    { output = \"copyR:Out\"  input = \"mixR:In 1\" }\n";
    ts << "                    { output = \"copyL:Out\"  input = \"delayL:In\" }\n";
    ts << "                    { output = \"delayL:Out\" input = \"mixR:In 2\" }\n";
    ts << "                ]\n";
    ts << "                inputs  = [ \"copyL:In\" \"copyR:In\" ]\n";
    ts << "                outputs = [ \"mixL:Out\" \"mixR:Out\" ]\n";
    ts << "            }\n";
    ts << "            audio.channels = 2\n";
    ts << "            audio.position = [ FL FR ]\n";
    ts << "            capture.props = {\n";
    ts << "                node.name   = \"effect_input.archtitan_spatial\"\n";
    ts << "                media.class = Audio/Sink\n";
    ts << "            }\n";
    ts << "            playback.props = {\n";
    ts << "                node.name    = \"effect_output.archtitan_spatial\"\n";
    ts << "                node.passive = true\n";
    ts << "            }\n";
    ts << "        }\n";
    ts << "    }\n";
    ...
    // Reload PipeWire, then route sink-inputs through the spatial sink
    QProcess::startDetached("bash", {"-c",
        "systemctl --user restart pipewire pipewire-pulse 2>/dev/null; "
        "sleep 0.8; "
        "pactl list short sink-inputs 2>/dev/null | awk '{print $1}' | "
